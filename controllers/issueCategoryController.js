  ...
  update: function (req, res) {
    ...
    try {
      issueCategoryModel.findOne({ _id: id }, function (err, issueCategory) {
        ...
        issueCategory.save(function (err, projeectCategory) {
          if (err) {
            const LOGMESSAGE = DATETIME + "|" + err.message;
            log.write("ERROR", LOGMESSAGE);
            return res.status(500).json({
          ...
          log.write("INFO", LOGMESSAGE);
          return res.json({success:true,msg:"issue Category is updated",data:issueCategory});
          // return res.json(issue Category);
        });
      });
    } catch (error) {
      const LOGMESSAGE = DATETIME + "|" + error.message;
