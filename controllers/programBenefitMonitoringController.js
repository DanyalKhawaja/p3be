...
module.exports = {
  ...
  chartByProgram: async function (req, res) {
    try {
      const DATETIME = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
      var id = req.params.id;
      let planned = await programBenefitModel.aggregate([
        { $match: { program: ObjectId(id) } },
        { $project: { benefit: "$benefit", name: "$description", from: "$benefitStartDate", to: { $dateAdd: { startDate: "$benefitStartDate", unit: "month", amount: "$benefitDuration" } }, planned: "$amount" } }
      ]);
      let actual = await programBenefitMonitoringModel.aggregate([
        { $lookup: { from: "programbenefits", localField: "benefit", foreignField: "_id", as: "Benefit" } },
        { $unwind: { preserveNullAndEmptyArrays: true, path: "$Benefit" } },
        { $match: { "Benefit.program": ObjectId(id) } },
      
        {
          $group: {
            _id: {
              ...
              name: "$description",
              from: "$benefitFrom",
              to: { $dateAdd: { startDate: "$benefitFrom", unit: "month", amount: "$benefitDuration" } }
            },
            actual: { $sum: "$benefitValue" }
          }
        }
      ]);
    ...
    showByProgram: async function (req, res) {
      ...
      try {
        ...
        let data = (await programBenefitMonitoringModel.aggregate([
          { $lookup: { from: "programbenefits", localField: "benefit", foreignField: "_id", as: "Benefit" } },
          {
            $unwind:
            {
          ...
          { $match: { "Benefit.program": ObjectId("5fff0c788ff5fa60f016a820") } },
          { $group: { _id: { benefit: "$benefit", name: "$Benefit.description", planned: "$Benefit.value" }, actual: { $sum: "$benefitValue" } } }]))
